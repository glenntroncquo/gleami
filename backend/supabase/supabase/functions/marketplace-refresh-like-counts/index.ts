...
import { refreshLikeCounts } from "../_shared/marketplace/like-counts.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const updated = await refreshLikeCounts(getMarketplaceDb());
    console.log(JSON.stringify({
      msg: "marketplace-refresh-like-counts",
      updated,
      elapsedMs: Date.now() - started,
    }));
    return new OkResponse({ ok: true, updated });
  } catch (err) {
    if (err instanceof UnauthenticatedError) return new BadResponse(err.message, 401);
    console.error(err);
    return new BadResponse("Like count refresh failed", 500);
  }
});
