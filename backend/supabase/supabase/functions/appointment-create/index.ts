...
import { createAppointmentSchema } from "../_shared/appointment/commands/create-with-referral/schema.ts";
import { createAppointmentHandler } from "../_shared/appointment/commands/create-with-referral/handler.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const validatedInput = validateInput(createAppointmentSchema, body);
    if (validatedInput instanceof BadResponse) {
      return validatedInput;
    }

    const result = await createAppointmentHandler(validatedInput);
